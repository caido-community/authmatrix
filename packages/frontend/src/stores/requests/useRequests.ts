...
export const useRequests = (context: Context) => {
  ...
  const toggleRequestRole = async (requestId: string, roleId: string) => {
    ...
    if (newRequest) {
      if (context.state.type === "Success") {
        const newRequests = context.state.requests.map(request => request.id === newRequest.id ? newRequest : request);
        context.state = {
          type: "Success",
  ...
  const deleteRequest = async (id: string) => {
    ...
    if (context.state.type === "Success") {
      const newRequests = context.state.requests.filter(request => request.id !== id);
      context.state = {
        type: "Success",
        requests: newRequests
      }
